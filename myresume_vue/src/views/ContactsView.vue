<template>
  <div class="contacts absolute top-20 w-full">
    <h1 class="text-4xl mb-6 pl-10">Contact</h1>
      ...
      <div class="basis-1/2 px-10">
        ...
        <form action="" @submit.prevent="giveFeedback">
          <input
            type="text"
            placeholder="Name"
            class="rounded w-3/4 h-10 mb-4 focus:outline-0 pl-4"
            v-model="name"
          />
          <input
            type="text"
          ...
          <textarea
            ...
            cols="40"
            rows="5"
            class="rounded w-3/4 mb-4 focus:outline-0 pl-4"
            v-model="message"
          ></textarea>
          ...
          <button
            class="rounded w-3/4 h-10 bg-slate-600 text-white font-bold hover:bg-white/50 hover:text-slate-600"
            type="submit"
          >
            SEND
          </button>
  ...
  methods: {
    async giveFeedback() {
      ...
      if (!this.errors.length) {
        ...
        await this.axios
          .post("/api/v1/employer-list/", formData)
          .then((response) => {
